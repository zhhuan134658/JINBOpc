<template>
    <div class="imgUpload" id="mpupload">
        <div id="pdfcontainer">
            <a id="pdfselectfiles-two" href="javascript:void(0);" class="btn">
                <el-button type="primary" plain>选择文件</el-button>
            </a>
        </div>
        <div><span style="color: red">*</span>可上传PDF文件</div>
        <ul id="ossfilea" class="ossfile" style="display: block">
          <template v-if="newFiles.length">
            <li id="arr_li" v-for="(item, index) in licenceImg">
                <div class="tukuang">
                    <!-- <img class="imgge" style="width:86px;height: 60px;float:left" :src="item" /> -->
    ...
    methods: {
        ...
        initPlUploader() {
            ...
            uploader = new plupload.Uploader({
                ...
                browse_button: 'pdfselectfiles-two',
                ...
                init: {
                        ...
                        if (uploader.files.length <= 9) {
                            plupload.each(files, function (file) {
                                if (uploader.files.length != 0) {
                                    ...
                                    _this.$alert('请添加PDF文件');
                                }
                            });
                        } else {
                            _this.$alert('最多只能上传9个PDF文件');
                    ...
                    FileUploaded: function (up, file, info) {
                        if (info.status == 200) {
                            ...
                            console.log(_this.newFiles, "newFiles");
