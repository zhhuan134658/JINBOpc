        ...
        >
            <div>
                ...
                >
                    ...
                    >
                        <el-select
                            v-model="manyForm.canjianzzcode"
            ...
            <div>
                ...
                >
                    ...
                    >
                        <el-input
                            placeholder="请输入参建单位信用代码"
                            v-model="editForm.bzxinyongcode"
                        ></el-input>
                    </el-form-item>
                    <el-form-item
                        class="marginb"
                    ...
                    >
                        <el-select
                            v-model="editForm.canjianzzcode"
                            placeholder="请选择参建单位类型"
                        >
                            <el-option
                                v-for="item in typeList"
    ...
    methods: {
        ...
        upBtn(formName) {
            ...
            that.$refs[formName].validate((valid) => {
                if (valid) {
                    axios
                        .post('/laowu_yun/JiaXingCanJianAdd', that.manyForm)
                        .then((res) => {
                            if (res.data.status == 1) {
                                that.loaded = false;
        ...
        editBtn(formName) {
            ...
            that.$refs[formName].validate((valid) => {
                if (valid) {
                    axios
                        .post('/laowu_yun/JiaXingCanJianAdd', that.editForm)
                        .then((res) => {
                            if (res.data.status == 1) {
                                that.loaded = false;
                                // that.$refs.manyEditForm.resetFields();
