...
export default {
    ...
    data() {
        return {
            ...
            options: [
                { name: '行政服务中心建委窗口', id: '行政服务中心建委窗口' },
                { name: '瑶海区建设局', id: '瑶海区建设局' },
                { name: '肥西县建设局', id: '肥西县建设局' },
                { name: '肥东县建设局', id: '肥东县建设局' },
                { name: '长丰县建管局', id: '长丰县建管局' },
                { name: '蜀山区建设局', id: '蜀山区建设局' },
                { name: '新站区建发局', id: '新站区建发局' },
                { name: '包河区建设局', id: '包河区建设局' },
                {
                    name: '经济开发区建设管理中心',
                    id: '经济开发区建设管理中心'
                },
                { name: '高新区建发局', id: '经济开发区建设管理中心' },
                { name: '庐阳区建设局', id: '庐阳区建设局' },
                { name: '市建委政务区工作站', id: '市建委政务区工作站' },
                { name: '巢湖市住建局', id: '巢湖市住建局' },
                { name: '庐江县建设局', id: '庐江县建设局' },
                {
                    name: '合肥市城乡建委巢湖经开区建设分局',
                    id: '合肥市城乡建委巢湖经开区建设分局'
                },
                { name: '合肥市城乡建设局', id: '合肥市城乡建设局' }
            ]
        };
    },
    ...
    methods: {
        ...
        handleBanzu(index, row) {
            if (!this.manyForm.zongbaoname) {
                this.$alert('请先修改接口！');
            } else {
                this.$store.commit('setbpXmTable', row.address);
                this.$store.commit('setbpXmCode', this.manyForm.zongbaoname);
                this.$router.push({
                    path: '/bphefeishibz',
                    query: {
                        xmid: row.xmid,
                        prcode: this.manyForm.zongbaoname,
                        address: row.address
                    }
                });
            }
    ...
    mounted() {
        if (this.xmid) {
            ...
            this.viwePage = true;
            this.$message({
                message: '上报前请上传人员的打卡照片，如有疑问请联系服务商',
                type: 'success'
            });
            this.getTable();
        }
